...
end evals = 1 samples = 1 gcsample = true setup = ($x .= 0.0; $r .= 0.5; $p .= 0.5; cond = 1.0)
end
=#
...
end seconds = 200 samples = 100 evals = 1 gcsample = true setup = (CUDA.device!(1); $x .= 0.0; CUDA.device!(0); $r .= 0.5; $CUDA.device!(1); $p .= 0.5; cond = 1.0)
end

results = run(suite)

writedlm("results $(now())", ["cg_async"].times, ',')
